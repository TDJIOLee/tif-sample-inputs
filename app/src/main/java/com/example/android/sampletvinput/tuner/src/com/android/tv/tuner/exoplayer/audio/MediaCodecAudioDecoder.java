...
public class MediaCodecAudioDecoder extends AudioDecoder {
    ...
    private static List<MediaCodecInfo> getDecoderInfo(MediaCodecSelector selector, String mimeType) { //DecoderInfo
        try {
            return selector.getDecoderInfos(mimeType, false);
    ...
    public void maybeInitDecoder(MediaFormat format) throws ExoPlaybackException {
        ...
        List<MediaCodecInfo> decoderInfo = getDecoderInfo(mSelector, mimeType); //DecoderInfo
        if (decoderInfo == null) {
            Log.i(TAG, "There is not decoder found for " + mimeType);
            return;
        }

        String codecName = decoderInfo.get(0).name;
        try {
    ...
    public void decode(SampleHolder sampleHolder) {
        if (mWaitingForFirstSyncFrame) {
            if (sampleHolder.flags != android.media.MediaExtractor.SAMPLE_FLAG_SYNC) { //if (!sampleHolder.isSyncFrame()) {
        ...
        if (sampleHolder.flags == com.google.android.exoplayer2.C.BUFFER_FLAG_DECODE_ONLY) { // if (sampleHolder.isDecodeOnly()) {
